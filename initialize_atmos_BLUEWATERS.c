...
static char vcid[] = "$Id: initialize_atmos.c,v 5.9.2.18 2012/08/29 00:10:56 vicadmin Exp $";

void initialize_atmos_BLUEWATERS(atmos_data_struct        *atmos,
                      dmy_struct               *dmy,
		      //FILE                    **infile,
...
//    nrerror("Input meteorological forcing files must contain either WIND (wind speed) or both WIND_N (north component of wind speed) and WIND_E (east component of wind speed); check input files\n");
  ...
  *******************************/
  ...
  forcing_data = read_forcing_data(global_param);
  //forcing_data = read_forcing_data(infile, global_param);

  /*#####################################################################
  Pass the forcing cell data to the old forcing data array (BLUE WATERS)
  ######################################################################*/

  for (i = 0; i < global_param.nrecs; i++){
    forcing_data[0][i] = forcing_cell[0].tair[i];
    forcing_data[6][i] = forcing_cell[0].lwdown[i];
    forcing_data[14][i] = forcing_cell[0].swdown[i];
    forcing_data[20][i] = forcing_cell[0].wind[i];
    forcing_data[9][i] = forcing_cell[0].prec[i];
    forcing_data[10][i] = forcing_cell[0].pres[i];
    forcing_data[11][i] = forcing_cell[0].shum[i];
  }
